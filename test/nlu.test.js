...
test('parseIntent recognises Jira assign/unassign/transition', () => {
  assert.equal(parseIntent('assign WSRU-1 to Fred', []).action, 'jira.assign');
...
test('execute(jira.assign) picks the best-scoring assignable user and reports ambiguity on a tie', async () => {
  const { client } = makeClient({
    scope: { jiraAssignableUsers: async () => [{ accountId: 'a1', displayName: 'Fred Kariuki' }, { accountId: 'a2', displayName: 'Fred Otieno' }] },
  });
  const r = await client.execute({ action: 'jira.assign', jiraKey: 'WSRU-1', who: 'Fred' });
  assert.match(r.message, /could be Fred Kariuki or Fred Otieno/);
});
