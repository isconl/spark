...
function createIdeasClient(opts) {
  ...
  function readIdeas() {
    return readTSV(ideasFile).map(r => ({
      ...r,
  ...
  function updateIdea(p) {
    if (!p.id) throw new Error('which idea?');
    let found = false;
    rewriteTSV(ideasFile, rows => rows.map(r => {
  ...
  function deleteIdea(id) {
    const removed = rewriteTSV(ideasFile, rows => rows.filter(r => r.ID !== id));
  ...
  function applyIdeaDirectives(reply, source = 'chat') {
    const text = String(reply || '');
    if (!text.includes('[[IDEA:')) return { text, captured: [] };
    const captured = [];
    const cleaned = text.replace(/\[\[IDEA:\s*([\s\S]*?)\]\]/gi, (_m, inner) => {
      try {
        const [title, body, type, domain] = String(inner).split('||').map(s => s.trim());
        if (!title) return '';
        const row = captureIdea({ title, body: body && body !== title ? body : '', type, domain, source });
        captured.push(row);
      } catch (e) {
