...
function createLearningClient(opts) {
  ...
  const DEFAULT_GROUPS = [
    ...
    {
      GROUP_ID: 'platforms-experience',
      LABEL: 'Platforms & Experience',
      DESCRIPTION: 'The B2B portals, publishing pipeline, and UX journeys -- ready the moment something is filed here.',
      ICON: '⚙️',
      COLOR: '#0d9488',
      STATUS: 'active',
      SORT_ORDER: '6',
    ...
    {
      GROUP_ID: 'profiles-psychology',
      LABEL: 'Profiles & Psychology',
      DESCRIPTION: 'Reading people -- personality, bias, behavioral patterns, influence, real-person profiles, and a full archetype field guide.',
      ICON: '🧠',
      COLOR: '#06b6d4',
      STATUS: 'active',
      SORT_ORDER: '7',
    ...
    {
      GROUP_ID: 'systems-architecture',
      LABEL: 'Systems & Architecture',
      DESCRIPTION: 'Systems thinking, software architecture, business-as-machine design, metrics that matter, and where scale breaks first.',
      ICON: '🏗️',
      COLOR: '#6366f1',
      STATUS: 'active',
      SORT_ORDER: '8',
    ...
    {
      GROUP_ID: 'projects-memory',
      LABEL: 'Projects & Memory',
      DESCRIPTION: 'Every project drive-wide -- progress, decisions and the reasoning behind them, open risks, and current state. The first thing an AI reads before advising Sconl on anything, from iSconl fleet work to Viva/corporate engagements to every venture.',
      ICON: '🛠️',
      COLOR: '#f97316',
      STATUS: 'active',
      SORT_ORDER: '9',
    ...
    {
      GROUP_ID: 'legal-compliance',
      LABEL: 'Legal & Compliance',
      DESCRIPTION: 'Kenyan and international law worth practical awareness of -- drug law, employment law, financial and tax regulation, and related subjects still being scoped. Track container only; the exact subject list is confirmed with Sconl before the first course is commissioned.',
      ICON: '⚖️',
      COLOR: '#64748b',
      STATUS: 'active',
      SORT_ORDER: '10',
    ...
    {
      GROUP_ID: 'identity-self',
      LABEL: 'Identity & Self',
      DESCRIPTION: 'Self-understanding and personal architecture -- a very helpful personal coach function, not a conventional course subject.',
      ICON: '🧭',
      COLOR: '#a855f7',
      STATUS: 'active',
      SORT_ORDER: '11',
    ...
    {
      GROUP_ID: 'brands-ventures',
      LABEL: 'Brands & Ventures',
      DESCRIPTION: "Understanding Sconl's own brand portfolio inside out and building each one from the ground up -- container for the per-brand courses.",
      ICON: '🏷️',
      COLOR: '#ec4899',
      STATUS: 'active',
      SORT_ORDER: '12',
  ...
  function defaultRelevanceFor(courseId, file) {
    // FL26091205: was 'viva-meetings', dead since the 3 Sep 2026 three-part
    // kebab rename (done-archive/2026-09.md's "Course ID Taxonomy
    // Standardization" entry) -- silently never matched, so this branch
    // never fired for any real course.
    if (courseId === 'viva-executive-meetings' && !file.includes('_00_') && !file.includes('_01_')) {
  ...
  // FL26091206: groups.tsv couldn't hold anything until FL26091204 fixed the
  // schema gap, so it has never actually been seeded on any real host --
  // every read fell through to DEFAULT_GROUPS. Auto-seed it from
  // DEFAULT_GROUPS on first real read (same "seed on empty" pattern
  // scope/blocks.tsv already uses), so a Track editor edit finally has a
  // real row to persist onto instead of a code constant nothing writes to.
  // DEFAULT_GROUPS stays as the genuine fallback for a host whose seed write
  // fails or hasn't run yet -- not re-synced by hand, per the row's own
  // "don't just re-sync the three lists by hand" warning.
  async function getRawGroups() {
    const raw = await readTSV('learning/groups.tsv');
    if (raw && raw.length) return raw;
    try { await rewriteTSV('learning/groups.tsv', () => DEFAULT_GROUPS.slice()); }
    catch { /* seed failed -- fall back to the constant below regardless */ }
    return DEFAULT_GROUPS;
  }

  // BL26091504: the five Campus bands, hoisted here so both campus() and
  // setModuleMeta() share one definition -- a module self-declaring
  // CAMPUS_BAND must validate against the exact same keys campus() renders,
  // never a second, drifting copy of the list.
  const CAMPUS_BANDS = [
  ...
  const CAMPUS_BAND_KEYS = CAMPUS_BANDS.map(b => b.key);
  const CAMPUS_DEPTH_KEYS = ['overview', 'deep-dive', 'reference'];
  const CAMPUS_TTL_DAYS_DEFAULT = 14;
  ...
  async function listCourses(opts = {}) {
    ...
    const courses = await Promise.all(coursesRaw.map(async c => {
      ...
      const lessons = files.map(({ file, raw, mtimeIso }) => {
        ...
        // BL26091504: a module's own Campus self-declaration, straight off
        // modules_meta.tsv -- empty CAMPUS_BAND means "no campus presence,"
        // never a guessed default. CAMPUS_COVERAGE_MINUTES defaults to the
        // same words/200 estimate the rest of this file already computes,
        // per the row's own "reuse, do not reinvent" instruction; a module
        // overrides it only where word count undercounts real effort.
        const campusBand = String(customMeta?.CAMPUS_BAND || '').toLowerCase();
        const campusCoverageMinutes = Number(customMeta?.CAMPUS_COVERAGE_MINUTES) || Math.max(1, Math.round(words / 200));
        ...
        return {
          ...
          campusBand: CAMPUS_BAND_KEYS.includes(campusBand) ? campusBand : '',
          campusNote: String(customMeta?.CAMPUS_NOTE || ''),
          campusExpiresAfterDays: Number(customMeta?.CAMPUS_EXPIRES_AFTER_DAYS) || CAMPUS_TTL_DAYS_DEFAULT,
          campusPriority: Number(customMeta?.CAMPUS_PRIORITY) || 99,
          campusDepth: CAMPUS_DEPTH_KEYS.includes(String(customMeta?.CAMPUS_DEPTH || '').toLowerCase()) ? String(customMeta.CAMPUS_DEPTH).toLowerCase() : '',
          campusCoverageMinutes,
          campusUpdatedAt: String(customMeta?.UPDATED_AT || ''),
        };
      });

      // c.GROUP_ID (from courses.tsv) is the only source of truth -- no
      // CLASSROOM-keyword fallback. GROUP_ID is populated on every course
      // row now; a course shipped without one surfaces as visibly
      // 'ungrouped' rather than being silently guessed into the wrong
      // track (BL26083101 -- that guessing was the actual production bug).
      const groupId = c.GROUP_ID || 'ungrouped';
  ...
  async function getLesson(course, file) {
    ...
    // BL26091005: reuse listLessonFiles' own per-entry mtimeIso (already
    // computed for the course's file listing) rather than adding a new
    // stat call -- one extra listDir per lesson read, same source of
    // truth as everything else that already trusts mtimeIso.
    let updatedAt = null;
    try {
      const entry = (await listLessonFiles(course)).find(e => e.file === file);
      updatedAt = entry ? entry.mtimeIso : null;
    } catch { /* updatedAt stays null -- client falls back to no date */ }
    return { content, blocks, updatedAt };
  }
  ...
  async function setModuleMeta(p) {
    ...
    // BL26091504: the campus fields are validated but NOT force-defaulted
    // the way RELEVANCE/STATUS/etc above are -- those are unconditionally
    // overwritten on every save regardless of whether the caller mentioned
    // them, which is fine for fields every caller always sends. Campus
    // fields are optional and set independently (e.g. by a different UI
    // than the one that sets RELEVANCE), so a save that omits them must
    // fall back to whatever the row already has, not silently wipe them
    // back to blank/defaults.
    if (p.campusBand !== undefined && p.campusBand !== '' && !CAMPUS_BAND_KEYS.includes(String(p.campusBand).toLowerCase())) {
      throw new Error(`campusBand must be one of ${CAMPUS_BAND_KEYS.join(', ')}, or empty`);
    }
    if (p.campusDepth !== undefined && p.campusDepth !== '' && !CAMPUS_DEPTH_KEYS.includes(String(p.campusDepth).toLowerCase())) {
      throw new Error(`campusDepth must be one of ${CAMPUS_DEPTH_KEYS.join(', ')}, or empty`);
    }
    if (p.campusPriority !== undefined && p.campusPriority !== '' && (Number(p.campusPriority) < 1 || Number(p.campusPriority) > 5)) {
      throw new Error('campusPriority must be between 1 and 5, or empty');
    }
    ...
    await rewriteTSV('learning/modules_meta.tsv', rows => {
      const updated = rows.map(r => {
        if (r.COURSE_ID === courseId && r.LESSON_FILE === lessonFile) {
          ...
          return {
            ...
            CAMPUS_BAND: p.campusBand !== undefined ? String(p.campusBand).toLowerCase() : (r.CAMPUS_BAND || ''),
            CAMPUS_NOTE: p.campusNote !== undefined ? String(p.campusNote).trim() : (r.CAMPUS_NOTE || ''),
            CAMPUS_EXPIRES_AFTER_DAYS: p.campusExpiresAfterDays !== undefined ? String(p.campusExpiresAfterDays) : (r.CAMPUS_EXPIRES_AFTER_DAYS || ''),
            CAMPUS_PRIORITY: p.campusPriority !== undefined ? String(p.campusPriority) : (r.CAMPUS_PRIORITY || ''),
            CAMPUS_DEPTH: p.campusDepth !== undefined ? String(p.campusDepth).toLowerCase() : (r.CAMPUS_DEPTH || ''),
            CAMPUS_COVERAGE_MINUTES: p.campusCoverageMinutes !== undefined ? String(p.campusCoverageMinutes) : (r.CAMPUS_COVERAGE_MINUTES || ''),
          };
        }
        return r;
      });
      if (!found) {
        updated.push({
          ...
          REVIEWED_AT: reviewedAt,
          UPDATED_AT: updatedAt,
          CAMPUS_BAND: p.campusBand !== undefined ? String(p.campusBand).toLowerCase() : '',
          CAMPUS_NOTE: p.campusNote !== undefined ? String(p.campusNote).trim() : '',
          CAMPUS_EXPIRES_AFTER_DAYS: p.campusExpiresAfterDays !== undefined ? String(p.campusExpiresAfterDays) : '',
          CAMPUS_PRIORITY: p.campusPriority !== undefined ? String(p.campusPriority) : '',
          CAMPUS_DEPTH: p.campusDepth !== undefined ? String(p.campusDepth).toLowerCase() : '',
          CAMPUS_COVERAGE_MINUTES: p.campusCoverageMinutes !== undefined ? String(p.campusCoverageMinutes) : '',
        });
      }
      return updated;
    });
  ...
  async function campus() {
    ...
    // FL26091205: the short-lesson-name indexing branch that used to sit
    // here matched a filename shape (`2026MMDD_campus_<track>_NN_...`) no
    // live module has ever used -- real files are
    // `YYYYMMDD_<track>_<course>_<module>_vX_Y_Z.md` (done-archive/
    // 2026-09.md's rename record) with no "campus" segment at all. Silently
    // dead since it was written; removed rather than guessed at a
    // replacement regex with no live filename sample to confirm it against.
    ...
    // BL26091504: module self-declared Campus items, merged alongside the
    // hand-curated campus.tsv rows above -- the whole point of this row is
    // that a module's own metadata can put it on the board with zero
    // separately-authored campus.tsv row. Expiry mirrors campus.tsv's own
    // TRIGGER=always rule above: RELEVANCE=evergreen pins it permanently
    // (same exemption `rowLive()` already gives an `always`-triggered
    // campus.tsv row referencing an evergreen module), otherwise it ages
    // out campusExpiresAfterDays (default 14) days after the module's own
    // modules_meta.tsv UPDATED_AT -- no date to age against means don't
    // guess it stale, same as `rowLive()`'s own fallback.
    const moduleItemsByBand = {};
    for (const c of live) {
      for (const l of (c.lessons || [])) {
        if (!l.campusBand) continue;
        if (l.relevance !== 'evergreen') {
          const updated = l.campusUpdatedAt;
          if (/^\d{4}-\d{2}-\d{2}$/.test(updated)) {
            const expiry = new Date(`${updated}T00:00:00`);
            expiry.setDate(expiry.getDate() + (l.campusExpiresAfterDays || CAMPUS_TTL_DAYS_DEFAULT));
            if (today > expiry.toISOString().slice(0, 10)) continue; // expired
          }
        }
        (moduleItemsByBand[l.campusBand] = moduleItemsByBand[l.campusBand] || []).push({
          courseId: String(c.ID), courseTitle: String(c.TITLE || c.ID),
          lesson: l.file, title: String(l.title || l.file), headline: '',
          why: l.campusNote || '', minutes: l.campusCoverageMinutes,
          status: doneOf[`${c.ID}/${l.file}`] || 'new', missing: false,
          depth: l.campusDepth || '',
          priority: l.campusPriority,
        });
      }
    }
    // CAMPUS_PRIORITY is 1-5, lower number = read first (same convention as
    // a P1/P2 ticket) -- unset modules default to 99 in listCourses() so
    // they always sort after anything explicitly prioritized.
    for (const arr of Object.values(moduleItemsByBand)) arr.sort((a, b) => a.priority - b.priority);

    let bands = BANDS
      .map(b => ({ ...b, items: [
        ...rows.filter(r => String(r.BAND || '').toLowerCase() === b.key).map(itemFrom),
        ...(moduleItemsByBand[b.key] || []),
      ] }))
    ...
    if (!bands.length || !bands.some(b => b.key === 'now' && b.items.length)) {
      ...
      // FL26091205: all 5 course IDs were pre-3-Sep-2026-rename and dead
      // (done-archive/2026-09.md's "Course ID Taxonomy Standardization"),
      // so campus()'s computed fallback silently produced nothing for any
      // of them -- `if (!c) continue;` below swallowed every miss with no
      // error. Updated to the real post-rename IDs.
      const priorityPointers = [
        { courseId: 'viva-executive-meetings', file: '08', why: 'Sam alignment: team vetting on HR portal, Egypt/Turkey mapping, CDN latency.' },
        { courseId: 'viva-twin-portals', file: '20', why: 'The full deal flow end-to-end: surfaces, roles, testing board, and live findings.' },
        { courseId: 'viva-business-model', file: '03', why: 'The 9 competitors analyzed: why trust verification is the product.' },
        { courseId: 'wellspring-publishing-pipeline', file: '02', why: 'Hero image composition rules: search the message, text on left, subject on right.' },
        { courseId: 'applied-financial-intelligence', file: '01', why: 'Sovereign wealth compounding: master capital allocation across all ventures.' },
      ];
  ...
  async function contributions() {
    ...
    // BL26091111: on a local dev machine this instance's own audit log is
    // nearly empty -- almost all real reading happens on live -- so the map
    // rendered as a blank year while live's looked right. Per Sconl: "the live
    // database happens to count the activity, the data that drives the map in
    // the live version is the same that should be used to display in the
    // local."
    //
    // sync-learning-from-live.js mirrors live's own /learning/contributions
    // into learning/contributions.tsv. If that mirror exists, it IS the
    // answer. It is only ever written by that script, which only ever runs
    // locally, so on live this file does not exist and the audit-log path
    // below stays the real source. No environment check needed: the presence
    // of the mirror is the signal.
    try {
      const mirrored = await readTSV('learning/contributions.tsv');
      if (mirrored.length) {
        for (const r of mirrored) {
          const dt = String(r.DATE || '').slice(0, 10);
          if (countsByDate[dt] !== undefined) countsByDate[dt] = Number(r.COUNT) || 0;
        }
        return { days: Object.entries(countsByDate).map(([date, count]) => ({ date, count })), source: 'live-mirror' };
      }
    } catch { /* no mirror -- fall through to this instance's own audit log */ }
