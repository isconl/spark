...
function createNluClient(opts) {
  ...
  function parse(text) {
    return parseIntent(text, readTSV(tasksFile));
  }
  ...
  async function execute(p) {
    switch (p.action) {
      ...
      case 'idea.create': {
        const row = ideas.captureIdea({ title: stripDashes(p.title), body: p.body ? stripDashes(p.body) : '', type: p.type, domain: p.domain, source: 'chat' });
