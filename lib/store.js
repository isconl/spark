...
// Whatever the underlying cause, a short retry makes the failure mode a
// non-issue for real usage -- this is reasonable resilience for a
// same-host sibling-service call regardless of why it's needed.
async function requestWithRetry(url, lib, method, data, getToken, { retries = 2, delayMs = 200 } = {}) {
  let lastErr;
